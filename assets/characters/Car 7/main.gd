...
@export var rare_spawn_points_group: NodePath = "RareSpawnPoints"
@export var rare_entity_scene: PackedScene
@export var rare_spawn_chance: float = 0.1  # 10% chance, can be changed dynamically
@export var rare_entity_min_active_threshold: int = 1
...
func rare_spawn_points := get_node(rare_spawn_points_group).get_children()
	_ready():
	randomize()
...
func _on_spawn_timer():
	if spawn_entities.is_empty() or spawn_points.is_empty() or game_over:
		return
		
	if is_special_active:
		return  # Don't spawn normal entities while rare entity is active
		
	# Check if we should spawn rare entity
	if randf() < rare_spawn_chance and not is_special_active:
		if get_poofable_count() <= rare_entity_min_active_threshold:
			spawn_special_entity()
			return  # Skip normal spawning this frame
		
	var num_to_spawn = randi_range(1, 3)
	for i in range(num_to_spawn):
		var entity_scene = spawn_entities
	if not is_rare_active and randi() % 100 < int(rare_spawn_chance * 100):
		if rare_entity_scene and rare_spawn_points.size() > 0:
			var rare_entity = rare_entity_scene.instantiate()
			var rare_spawn_point = rare_spawn_points[randi() % rare_spawn_points.size()]
			rare_entity.global_transform.origin = rare_spawn_point.global_transform.origin
			add_child(rare_entity)
			is_rare_active = true
.pick_random()
...
func get_poofable_count() -> int:
	var count := 0
	for child in get_children():
		if child.is_in_group("poofable"):
			count += 1
	return count
	
func spawn_special_entity():
	if not rare_entity_scene:
		return

	print("Spawning rare entity (e.g. airplane)!")
	is_special_active = true

	# Pick a random spawn lane
	var lane_index = randi_range(0, spawn_points.size() - 1)
	var spawn_point = spawn_points[lane_index]

	var rare_entity = rare_entity_scene.instantiate()
	if rare_entity is CharacterBody3D:
		rare_entity.global_transform = spawn_point.global_transform
		rare_entity.speed = max(spawn_speed + randf_range(-spawn_speed_variance, spawn_speed_variance), 0.1)
		rare_entity.lane_index = lane_index
		add_child(rare_entity)
	else:
		push_error("Rare entity is not a CharacterBody3D")
